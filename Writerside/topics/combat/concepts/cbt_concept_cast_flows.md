...
A **Cast Flow** describes how the ability resolves the cast. Each flow uses the same base ability, but changes how targets
...
A **Cast Actor** is used when the ability needs to place an actor in the world. Examples include a lava pool, aura,
explosion volume, meteor marker, lingering hazard, or healing zone.
...
**Cast Behaviors** are reusable rules hosted by the Cast Actor. They let Cast Actors handle placement, target interaction,
effects, cosmetics, and dissipation without placing every rule directly on the actor class.

Common Cast Behaviors include:

| Behavior                             | Purpose                                                                                  |
|--------------------------------------|------------------------------------------------------------------------------------------|
| **Random Placement**                 | Places the cast at a random location around its owner.                                   |
...
| **Cosmetics**                        | Executes Gameplay Cues for selected cast events on clients.                              |
...
The **Random Placement** behavior is useful for casts that should appear near the source actor instead of exactly on top of
it. It can be configured with a radius range and is usually placed before **Floor Alignment**, so it chooses the horizontal
location first and floor alignment resolves the final ground position.

By default, Random Placement should not replace explicit placements. This preserves flows where the cast location was
already selected by the player, a Targeting Actor, or a scripted request.
...
>
> Use the Cast Cosmetics Behavior only when you need lightweight Gameplay Cues tied to cast events.
{style="tip"}

## Cast Ability

The **Cast Ability** coordinates the Cast Flow. Depending on its configuration, it can collect targets directly, wait for
target confirmation, spawn a Cast Actor, or combine targeting and spawning into the same flow.
...
| Trigger                | Description                                                           |
|------------------------|-----------------------------------------------------------------------|
| **Ability Activation** | The cast starts as soon as the Gameplay Ability activates.            |
| **Gameplay Event**     | The cast starts when the ability receives the cast Gameplay Event.    |
| **Delayed**            | The cast starts after a configured delay for non-interactive flows.   |
...
|-----------------------|------------------------------------------------------------------------|
| **Ability Activates** | Costs and cooldowns are committed when the Gameplay Ability activates. |
...
casts own their world presence, collision, duration, placement, and target interactions.
...
| **Loop**      | Looped section played while waiting for target confirmation.    |
...
Cast Requests are usually created by the **Trigger Cast** Animation Notify and sent to the ability through the cast Gameplay
Event. The ability reads the request from the event payload and uses it to continue the Cast Flow.

A Cast Request can also provide the cast placement and spawn or retrieve a Cast Actor, including support for the
[**Actor Pool**](cbt_concept_actor_pooling.md).
