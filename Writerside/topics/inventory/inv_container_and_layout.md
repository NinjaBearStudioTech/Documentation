...
<tldr>
    <ul>
        ...
        <li>Containers are defined by a <b>Container Definition</b> (<code>UNinjaInventoryContainerDataAsset</code>), and represented by a <b>Container Instance</b> (<code>UNinjaInventoryContainer</code>).</li>
        <li>Containers must have their definition data asset added to the <b>Asset Manager</b>.</li>
        <li>Layouts are defined by <code>UNinjaInventoryLayoutDataAsset</code>, determine all containers initially available to an Inventory Manager, and which one is the <b>default container</b>.</li>
        <li>Layouts can be provided to an Inventory Manager via the <b>default property value</b>, or via the <code>IInventoryManagerProviderInterface</code>.</li>
    </ul>
</tldr>
...
Default containers are important, as they assist the Inventory Manager to select containers for new items when needed.
They are usually identified by the `Inventory.Container.Default` Gameplay Tag, present in their `Gameplay Tags` property.

Then, the [Inventory Layout](#inventory-layouts) must define a Gameplay Tag Query to identify such items, allowing some
...
and off-hands, each hand being represented by their own containers.

An item can only ever have one primary container, but it can have multiple secondary ones. The relationship between 
primary and their relative secondary containers is defined in the Container Definition.

### Automatic Container and Position Assignments

Items added to the inventory can define their preferred container and position. However, these request might not be
viable and then a new position and container might be necessary. This is also the case for items that are added to the
inventory without defining these values.

For those situations, the Inventory Manager will attempt to determine the most appropriate container and position that
should be assigned to the new Item. This is based on the configuration available for each container, ordering them by
their priorities.

If an automatic assignment is not possible, then other aspects of the item might be triggered, such as the **Drop Fragment**
that, if present, would drop the item as a Pickup Actor.
