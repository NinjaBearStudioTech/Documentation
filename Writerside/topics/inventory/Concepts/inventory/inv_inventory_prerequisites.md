...
- **Adding Items to Inventory**: If the [**Prerequisites Fragment**](inv_inventory_fragments.md) is present in the item, it will be used to evaluate if the item can be added to the inventory.
- **Equipping an Item**: If the item has an [**Equipment Representation**](inv_inventory_fragments.md), it may include prerequisites that will determine if the item can be equipped or not. 
