...
>
> You can subclass `NinjaInventoryEquipment` to implement custom logic and assign your subclass directly to the [**Equipment Fragment**](inv_inventory_fragments.md).
...
This component receives a reference to the `NinjaInventoryEquipment` instance that spawned it and provides support functions  
...
These are configured in the **Equipment Fragment** and mapped using one or more **Gameplay Tags**. Not all equipment 
states need to be represented visually, only those listed in the fragment will be created.
...
- **[Equipment Fragment](inv_inventory_fragments.md)**: Provides the `SetEquipmentState` function that can be used to update the item's equipment state.
