...
<tldr>
    <ul>
        ...
        <li>Interaction Targets will only accept a source only if they have at least one <b>compatible interaction</b>.</li>
        <li>Once the first target is registered, the source <b>continuously scans</b> for the best candidate (e.g., by distance or camera aim) and applies <b>focus</b>; scanning stops when no targets are available.</li>
        ...
        <li>The source runs a <b>State Tree</b> (move/rotate, play animation, etc.), which eventually executes any <b>specific behavior</b> on the target and produces an outcome.</li>
    </ul>
</tldr>
...
specialized logic and flows, integrated with other parts of the Interaction System. The target is represented by the 
presence of an **Interaction Target Component**.
...
these targets can be **focused**, making them eligible for the **Interaction Flow**. The source is represented by the
presence of an **Interaction Manager Component**.

## Flow Overview
The following steps are involved in the **interaction flow**:

<img src="int_end_to_end_flow.png" alt="End-to-End Interaction Flow" width="800"/>
